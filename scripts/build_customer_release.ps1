# Builds a release APK for the Customer app.
# Run from the repo root:  .\scripts\build_customer_release.ps1
# Requires: Flutter SDK on PATH, Android SDK configured.

$ErrorActionPreference = 'Stop'
$repoRoot = Resolve-Path (Join-Path $PSScriptRoot '..')
$appDir   = Join-Path $repoRoot 'apps\customer_app'
$outDir   = Join-Path $appDir 'build\app\outputs\flutter-apk'

Write-Host "==> Cleaning customer_app" -ForegroundColor Cyan
Set-Location $appDir
flutter clean
...
Write-Host "==> Building release APK (customer_app)" -ForegroundColor Cyan
