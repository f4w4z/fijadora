# Builds a release APK for the Staff app.
# Run from the repo root:  .\scripts\build_staff_release.ps1
# Requires: Flutter SDK on PATH, Android SDK configured.

$ErrorActionPreference = 'Stop'
$repoRoot = Resolve-Path (Join-Path $PSScriptRoot '..')
$appDir   = Join-Path $repoRoot 'apps\staff_app'
$outDir   = Join-Path $appDir 'build\app\outputs\flutter-apk'

Write-Host "==> Cleaning staff_app" -ForegroundColor Cyan
Set-Location $appDir
flutter clean
...
Write-Host "==> Building release APK (staff_app)" -ForegroundColor Cyan
