...
import 'package:phoebe_core/data/repositories/jobs_repository.dart';
import 'package:phoebe_core/data/repositories/shop_repository.dart';
import 'package:phoebe_core/data/services/app_notification_service.dart';
import 'package:phoebe_core/data/services/telemetry_service.dart';
import 'package:phoebe_core/domain/models/app_user.dart';
import 'package:phoebe_core/domain/models/job_status.dart';
import 'package:phoebe_core/domain/models/maintenance_job.dart';
import 'package:phoebe_core/domain/models/product.dart';
import 'package:phoebe_core/domain/models/trade_type.dart';
...
import 'package:phoebe_core/ui/features/services/view_models/jobs_view_model.dart';
import 'package:phoebe_core/ui/features/shop/view_models/cart_view_model.dart';
...
class TestAuthRepository implements AuthRepository {
  ...
  Future<AppUser> signUpWithEmail({required String email, required String password, required String name, required UserRole role}) async {
    ...
    _currentUser = AppUser(id: 'user-1', email: email, name: name, role: role, createdAt: DateTime(2026));
    _controller.add(_currentUser);
    return _currentUser!;
  }
...
void main() {
  group('Auth → Shop → Job Flow', () {
    ...
    test('Auth flow: sign up → sign in → sign out → sign in again', () async {
      final authVM = AuthViewModel(authRepo);

      // Sign up
      await authVM.signUp(email: 'user@test.com', password: 'pw123', name: 'Test', role: UserRole.customer);
      ...
      // Sign in again
      await authVM.signIn(email: 'user@test.com', password: 'pw123');
      expect(authVM.isAuthenticated, isTrue);
      expect(authVM.user?.email, 'user@test.com');
    ...
    test('Auth flow: sign in failure does not set user', () async {
      final authVM = AuthViewModel(authRepo);
      authRepo.setFailure(true);

      try { await authVM.signIn(email: 'bad@test.com', password: 'wrong'); } catch (_) {}
      expect(authVM.isAuthenticated, isFalse);
      expect(authVM.user, isNull);
      expect(authVM.errorMessage, contains('Sign in failed'));

      // Failure clears on next successful attempt
      authRepo.setFailure(false);
      await authVM.signIn(email: 'good@test.com', password: 'right');
      expect(authVM.isAuthenticated, isTrue);
